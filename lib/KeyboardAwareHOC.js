  ...
  return class
    ...
    _updateKeyboardSpace = (frames: Object) => {
      ...
      if (this.props.enableAutomaticScroll) {
        ...
        const responder = this.getScrollResponder()
        if (!currentlyFocusedInput || !responder) {
          return
        }
    ...
    update = () => {
      ...
      this._scrollToFocusedInputWithNodeHandle(currentlyFocusedInput)
    }
...
const listenToKeyboardEvents = (configOrComp: any) => {
  return KeyboardAwareHOC(configOrComp); // Support for RN 0.63
}
